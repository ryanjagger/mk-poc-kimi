# Kart Racer — Product Requirements Document
...
A browser-based multiplayer kart racer with an arcade, Mario Kart–style feel, synchronized with **rollback netcode**. Up to 4 players race in real time, rendered in 3D, using original assets only. This v1 is deliberately scoped to prove the hard part — a bit-deterministic simulation running under rollback — with minimal racing gameplay built around it.
...
The point of v1 is to validate that a bit-deterministic, fixed-point sim plus a rollback loop is achievable in a TypeScript/browser environment, then layer minimal racing on top. If determinism doesn't hold, nothing else matters — so it's the gate everything else depends on.

## 3. Users & context

A small-scale, friends-in-a-room product: players create or join a private room by code and race. Not a public matchmaking service. Desktop browser, keyboard input (v1).
...
- Realistic vehicle physics — arcade only.
- More than 4 players.
- Mobile, touch, or gamepad input.
- Multiple tracks or a track editor.
- More than one item type.
- Matchmaking, ranking, persistence, or accounts.
- Spectators or replays. (Deterministic sim makes replays cheap to add later — noted as future, not built now.)
- Anti-cheat hardening beyond what the chosen topology naturally provides.
- Audio or a full art pass.
